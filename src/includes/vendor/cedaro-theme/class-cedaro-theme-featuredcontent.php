...
class Cedaro_Theme_FeaturedContent {
	...
	public function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'featured_content', array(
			'title'       => esc_html__( 'Featured Content', 'cedaro-theme' ),
			'description' => sprintf( esc_html__( 'Your theme supports up to %d posts in its featured content area.', 'cedaro-theme' ), $this->max_posts ),
			'priority'    => 130,
		) );
	...
	public function print_templates() {
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-button-add">
			<button class="button button-secondary alignright"><?php esc_html_e( 'Add Posts', 'cedaro-theme' ); ?></button>
		</script>
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-post">
			...
			<div class="ctfc-post-inside">
				<div class="ctfc-post-actions">
					<a class="ctfc-post-remove js-remove"><?php esc_html_e( 'Delete', 'cedaro-theme' ); ?></a> |
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-modal">
			<div class="ctfc-modal-head find-box-head">
				<?php esc_html_e( 'Find Posts', 'cedaro-theme' ); ?>
			...
			<div class="ctfc-modal-inside find-box-inside">
				<div class="ctfc-modal-search find-box-search">
					...
					<input type="button" value="<?php esc_attr_e( 'Search', 'cedaro-theme' ); ?>" class="button ctfc-modal-search-button" />
					<div class="clear"></div>
				</div>
	...
	public function ajax_find_posts() {
		...
		if ( ! $posts ) {
			wp_send_json_error( esc_html__( 'No items found.', 'cedaro-theme' ) );
		}
	...
	protected function get_found_posts_html( $posts ) {
		...
		$html = sprintf(
			...
			esc_html__( 'Title', 'cedaro-theme' ),
			esc_html__( 'Type', 'cedaro-theme' ),
			esc_html__( 'Date', 'cedaro-theme' ),
			esc_html__( 'Status', 'cedaro-theme' )
		);

		foreach ( $posts as $post ) {
			$title = trim( $post->post_title ) ? $post->post_title : esc_html__( '(no title)', 'cedaro-theme' );
			$alt   = ( 'alternate' === $alt ) ? '' : 'alternate';

			switch ( $post->post_status ) {
				case 'publish' :
				case 'private' :
					$stat = esc_html__( 'Published', 'cedaro-theme' );
					break;
				case 'future' :
					$stat = esc_html__( 'Scheduled', 'cedaro-theme' );
					break;
				case 'pending' :
					$stat = esc_html__( 'Pending Review', 'cedaro-theme' );
					break;
				case 'draft' :
					$stat = esc_html__( 'Draft', 'cedaro-theme' );
					break;
			}
			...
			if ( '0000-00-00 00:00:00' === $post->post_date ) {
				...
				$time = mysql2date( esc_html__( 'Y/m/d' ), $post->post_date );
			}
